...
class GsonCustomClassesWithTypeArgs : BaseGsonTest() {

    /*@get:Rule
    var globalTimeout: Timeout = Timeout.millis(300)*/
    ...
    @Test
    fun typeArgsListIsAnnotated() {
        // List is an annotated class
        timeOf(300) {
            json = list1.toJson()
    ...
    @Test
    fun typeArgs1() {
        val list: List<*> = listOf(2, "s")

        json = gson.toJson(list)
        val from: List<*> = gson.fromJson(json, List::class.java)

        assertNotEquals(list, from)

        val json = list.toJsonOrNull()

        val from2 = json.fromJsonOrNull<List<*>>()

        assertNotEquals(list, from2)
    }
