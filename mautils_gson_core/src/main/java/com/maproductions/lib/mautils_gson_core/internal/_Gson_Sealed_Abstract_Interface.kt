...
import com.maproductions.lib.mautils_gson_core.getDeclaredFieldsForSuperclassesOnly
...
internal class JsonSerializerForSealedClasses : JsonSerializer<Any> {
            ...
            for (field in (classDeclaredFields + allSuperclassesDeclaredFields)) {
                ...
                if (field.type != fieldInstance?.javaClass) {
                    ...
                    if (isSubclassOfAnnotatedClasses) {
                        jsonValue = serialize(fieldInstance, field.type, context)?.toString()
                    }
    ...
    private fun serializeSpecialCase(
        ...
        return runCatching { JsonParser.parseString(innerJsonObject.toString()) }.getOrNull()
    }
